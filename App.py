...
def load_model():
  with open('hospital_modeld.pkl', 'rb') as f:
...
with st.form("triage_form"):
  ...
  with col_temp:
    temperature_level = st.selectbox("🌡️ Temperature", options=list(temp_map.keys()), index=1)
  with col_hr:
    heart_rate_level = st.selectbox("💓 Heart Rate Level", options=list(hr_map.keys()), index=1)
  ...
  with col_age:
    age = st.number_input("👶🧒🧑🧓 Age", min_value=0, max_value=1000, value=20)
  with col_gen:
    gender = st.selectbox("♂️♀️ Gender", options=['Female', 'Male', 'I Prefer Not To Say', 'Other'])
